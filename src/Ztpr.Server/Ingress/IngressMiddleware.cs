    ...
    ILogger<IngressMiddleware> logger)
{
    ...
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        if (!TryGetSubdomain(host, cfg.AppHost, cfg.BaseDomain, out var subdomain))
        {
            await next(context);
            return;
        }
        ...
        if (session is null)
        {
            await WritePlainAsync(context, StatusCodes.Status404NotFound,
                $"No active tunnel for '{subdomain}.{cfg.BaseDomain}'.");
            return;
        }
    ...
    private static bool TryGetSubdomain(string host, string appHost, string baseDomain, out string subdomain)
    {
        subdomain = "";
        if (string.Equals(host, appHost, StringComparison.OrdinalIgnoreCase))
            return false;

        var baseSuffix = "." + baseDomain;
        if (!host.EndsWith(baseSuffix, StringComparison.OrdinalIgnoreCase))
            return false;

        var label = host[..^baseSuffix.Length];
